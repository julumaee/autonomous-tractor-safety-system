...
class SafetyMonitor(Node):
    ...
    def __init__(self):
        ...
        self.detection_subscription = self.create_subscription(
            FusedDetection, "/fused_detections", self.control_speed_state, 10
        )
        self.agopen_subscription = self.create_subscription(
            ControlCommand, "/control/agopen", self.agopen_control, 10
        )
        ...
        self.detection_active_reset_time = self.get_parameter(
            "detection_active_reset_time"
        ).value
        self.vehicle_stopped_reset_time = self.get_parameter(
            "vehicle_stopped_reset_time"
        ).value
        ...
        self.vehicle_state = (
            "agopen"  # Vehicle state: 'agopen, moderate, slow or stopped'
        )
        ...
        if distance <= self.stop_distance:
            if self.vehicle_state != "stopped":
                self.get_logger().info(
                    f"Obstacle at {distance} m! Stopping the vehicle."
                )
            ...
            if self.vehicle_state != "stopped":
                if self.vehicle_state != "slow":
                    self.get_logger().info(
                        f"Detection at {distance} m, " "entering slow speed state."
                    )
            ...
            if self.vehicle_state not in ["stopped", "slow"]:
                if self.vehicle_state != "moderate":
                    self.get_logger().info(
                        f"Detection at {distance} m, " "entering moderate speed state."
                    )
    ...
    def state_control(self):
        ...
        if self.vehicle_state == "stopped":
            if (
                self.get_clock().now() - self.stop_time
            ).nanoseconds / 1e9 > self.vehicle_stopped_reset_time:
                self.vehicle_state = "slow"
                self.last_detection_time_2 = (
                    current_time  # Reset timer to delay speed change
                )
                self.get_logger().info(
                    "Vehicle has been stopped for "
            ...
            if time_diff_2 > self.detection_active_reset_time:
                self.vehicle_state = "moderate"
                self.last_detection_time_1 = (
                    current_time  # Reset timer to delay speed change
                )
                self.get_logger().info(
                    "No detections inside safety_distance_2 "
            ...
            if time_diff_1 > self.detection_active_reset_time:
                ...
                self.get_logger().info(
                    "No detections inside safety_distance_1 "
                    f"for {self.detection_active_reset_time} seconds. "
    ...
    def speed_control(self):
        ...
        if self.vehicle_state == "agopen":
            ...
            self.get_logger().info(
                "No active detections, forwarding AgOpenGPS control commands."
            )
