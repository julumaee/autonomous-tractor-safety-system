...
class SafetyMonitor(Node):
    def __init__(self):
        ...
        self.vehicle_state = "agopen"                       # Variables for monitoring vehicle state. Values are "agopen, moderate, slow and stopped"
        self.latest_steering_angle = 0                      # Latest steering angle from AgOpenGPS, used for stop commands
    ...
    def agopen_control(self, agopen_cmd):
        ...
        if self.vehicle_state == "stopped":
            if ((self.get_clock().now() - self.stop_time).nanoseconds / 1e9 > self.vehicle_stopped_reset_time):
                self.vehicle_state = "slow"
        ...
        if self.vehicle_state == "agopen":
            ...
            self.get_logger().info(f"Vehicle will remain stopped for at least {self.vehicle_stopped_reset_time - (self.get_clock().now() - self.stop_time).nanoseconds / 1e9} seconds.")
        elif self.vehicle_state == "slow":
            self.get_logger().info(f"Vehicle in state 'slow', overriding AgOpenGPS speed command to {self.speed_override_2}.")
            agopen_cmd.speed = self.speed_override_2
            self.publisher_.publish(agopen_cmd)
            self.latest_steering_angle = agopen_cmd.steering_angle
        elif self.vehicle_state == "moderate":
            self.get_logger().info(f"Vehicle in state 'moderate' , overriding AgOpenGPS speed command to {self.speed_override_1}.")
            agopen_cmd.speed = self.speed_override_1
            self.publisher_.publish(agopen_cmd)
            self.latest_steering_angle = agopen_cmd.steering_angle
        else:
            self.send_stop_command
            self.get_logger().info(f"Vehicle in unknown state. Stopping the vehicle.")
