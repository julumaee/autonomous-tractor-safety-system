...
def generate_launch_description():
    ...
    vehicle_cmd_topic = DeclareLaunchArgument(
        "vehicle_cmd_topic", default_value="/vehicle_cmd_vel"
    )

    # Camera topics (ROS side, after bridge)
    rgb_topic = DeclareLaunchArgument("rgb_topic", default_value="/sim/cam/rgb")
    depth_topic = DeclareLaunchArgument("depth_topic", default_value="/sim/cam/depth")
    camera_info_topic = DeclareLaunchArgument(
        "camera_info_topic", default_value="/sim/cam/camera_info"
    )

    # LiDAR topic (ROS side, after bridge)
    lidar_points_topic = DeclareLaunchArgument(
        "lidar_points_topic", default_value="/sim/lidar/points"
    )
    lidar_scan_topic = DeclareLaunchArgument(
        "lidar_scan_topic", default_value="/sim/lidar"
    )

    # Frames coming from SDF
    cam_opt_frame = DeclareLaunchArgument(
        "camera_frame", default_value="tractor_simple/camera_link/front_depth_optical"
    )
    radar_frame = DeclareLaunchArgument(
        "radar_frame", default_value="tractor_simple/lidar_link/front_lidar"
    )

    # Control chain topics
    cmd_vel_in = DeclareLaunchArgument("cmd_vel_in", default_value="/cmd_vel")
    nav_cmd_out_topic = DeclareLaunchArgument(
        "nav_cmd_out_topic", default_value="/control/agopen"
    )
    safety_out_topic = DeclareLaunchArgument(
        "safety_out_topic", default_value="/control"
    )
    ...
    scenario_arg = DeclareLaunchArgument(
        "scenario", default_value="", description="Scenario/world name"
    )
    ...
    bridge_args = [
        ...
        [
            ...
            TextSubstitution(
                text="@sensor_msgs/msg/PointCloud2@gz.msgs.PointCloudPacked"
            ),
        ],
        # clock (no LaunchConfiguration here)
        TextSubstitution(
            text="/world/simple_field/clock@rosgraph_msgs/msg/Clock@gz.msgs.Clock"
        ),
    ...
    return LaunchDescription(
        [
            ...
            spatial_out_topic,
            use_d2d_relay,
            start_teleop_bridges_arg,
            start_rviz_bridge_arg,
            # gazebo
            gz_sim,
            # nodes
            bridge_node,
