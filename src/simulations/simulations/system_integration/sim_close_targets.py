...
class SyntheticTargetsPublisher(Node):
    ...
    def _load_params(self):
        ...
        self.targetA = np.array([gp('targetA_x').value, gp('targetA_y').value], dtype=float)
        self.enable_B = bool(gp('enable_B').value)
        self.targetB = np.array([gp('targetB_x').value, gp('targetB_y').value], dtype=float)
    ...
    def _on_tick(self):
        ...
        # Publish RADAR A
        self._publish_radar_point(Ax + rad_noise[0], Ay + rad_noise[1],
                                  self.vr_A, tag='A')

        # Publish CAMERA A (with independent noise)
        self._publish_camera_point(Ax + cam_noise[0] + self.deviation_x,
                                   Ay + cam_noise[1] + self.deviation_y,
                                   conf=self.cam_conf, tag='A')
        ...
        if self.enable_B:
            ...
            # RADAR B
            self._publish_radar_point(Bx + rad_noise_B[0], By + rad_noise_B[1],
                                      self.vr_B, tag='B')

            # CAMERA B
            self._publish_camera_point(Bx + cam_noise_B[0], By + cam_noise_B[1],
                                       conf=self.cam_conf, tag='B')
