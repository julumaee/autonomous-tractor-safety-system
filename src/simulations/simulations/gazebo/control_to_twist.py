...
class ControlToVehicle(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "twist_topic", "/vehicle_cmd_vel"
        )  # sim input (if interface=twist)
        self.declare_parameter(
            "ack_topic", "/vehicle/ackermann_cmd"
        )  # if interface=ackermann
        ...
        self.declare_parameter(
            "steer_units_per_rad", 180.0 / math.pi
        )  # ints per 1 rad (deg → int)
        ...
        if self.iface == "twist":
            ...
            self.pub_ack = self.create_publisher(
                AckermannDriveStamped, self.ack_out, 10
            )
