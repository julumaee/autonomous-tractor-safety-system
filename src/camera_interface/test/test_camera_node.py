...
#    create_detection_array(self): Creates a simulated Detection3DArray message with         #
#    two detections.                                                                         #
...
class TestCameraNode(unittest.TestCase):
    ...
    def create_camera_detection(self, x, y, z, object_hypothesis):
        ...
        msg = Detection3D()
        msg.header = Header()
        msg.header.stamp = self.camera_node.get_clock().now().to_msg()
        ...
        result_with_pose.hypothesis = object_hypothesis
        # Set pose position to match the detection position
        result_with_pose.pose.pose.position = Point(x=x, y=y, z=z)
        msg.results = [result_with_pose]
        return msg

    def create_detection_array(self):
        """Create a simulated Detection3DArray message with 2 detections."""
    ...
    def test_camera_node(self):
        ...
        self.assertEqual(
            camera_detection_1.header.frame_id,
            "camera_link",
            msg="Detection 1 should have correct ID",
        )
        self.assertEqual(
            camera_detection_1.results[0].class_id,
            camera_msg.detections[0].results[0].hypothesis.class_id,
        ...
        self.assertEqual(
            camera_detection_2.header.frame_id,
            "camera_link",
            msg="Detection 2 should have correct ID",
        )
        self.assertEqual(
            camera_detection_2.results[0].class_id,
            camera_msg.detections[1].results[0].hypothesis.class_id,
