...
def plot_xy(
    ...
    def _format_track_id(track_id) -> str:
        try:
            as_float = float(track_id)
    ...
    def _plot_tracks_grouped(tracks: pd.DataFrame, *, annotate_if_many: bool = True):
        ...
        unique_ids = (
            tracks["tracking_id"].dropna().astype(object).unique().tolist()
        )
        unique_ids = sorted(unique_ids, key=_track_sort_key)
        n_tracks = len(unique_ids)
        if n_tracks == 0:
            return

        cmap = plt.get_cmap("tab20")
        max_legend_tracks = 20
        show_legend = n_tracks <= max_legend_tracks

        for idx, track_id in enumerate(unique_ids):
            track_rows = tracks.loc[tracks["tracking_id"] == track_id, ["x", "y"]].dropna()
            if track_rows.empty:
                continue

            color = cmap(idx % cmap.N)
            label = f"Track {_format_track_id(track_id)}" if show_legend else None
            plt.scatter(
                track_rows["y"],
                track_rows["x"],
                marker=".",
            ...
            if annotate_if_many and not show_legend:
                # Put a small label at the last point so tracks are still identifiable.
                last = track_rows.iloc[-1]
                plt.text(
                    float(last["y"]),
                    float(last["x"]),
                    _format_track_id(track_id),
                    fontsize=6,
                    color=color,
                    alpha=0.9,
                )
